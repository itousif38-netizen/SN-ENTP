...
import { Project, PurchaseEntry, StockConsumption } from '../types';
import { Package, MinusCircle, Search, Save, Trash2, ArrowRight } from 'lucide-react';
       ...
       {isFormOpen && (
           <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 animate-in slide-in-from-top-2 border-l-4 border-l-orange-500">
               <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                   <MinusCircle className="text-orange-500" size={20} /> Record Material Consumption (Stock Out)
               </h3>
               <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                   ...
                   <div>
                       ...
                       <input 
                           ...
                           required
                       />
                       <p className="text-[10px] text-slate-400 mt-1">Must match purchase name exactly.</p>
                   </div>
                   <div>
                   ...
                   <div>
                       <label className="block text-xs font-medium text-slate-500 mb-1">Used In Activity (Optional)</label>
                       <input 
                           type="text" 
                           className="w-full border p-2 rounded"
                           placeholder="e.g. 1st Floor Slab"
                   ...
                   <table className="w-full text-left text-sm">
                       <thead className="bg-slate-100 border-b border-slate-200">
                           <tr>
                               ...
                               <th className="px-6 py-3 font-semibold text-slate-700 text-right">In (Purchased)</th>
                               <th className="px-6 py-3 font-semibold text-slate-700 text-right">Out (Consumed)</th>
                       ...
                       <tbody className="divide-y divide-slate-200">
                           {inventoryData.map((item, idx) => (
                               <tr key={idx} className="hover:bg-slate-50">
                                   ...
                                   <td className="px-6 py-4 text-right text-green-700 font-medium bg-green-50/30">
                                       {item.purchased}
                                   </td>
                                   <td className="px-6 py-4 text-right text-orange-700 font-medium bg-orange-50/30">
                                       {item.consumed}
                                   </td>
                                   <td className="px-6 py-4 text-right font-bold text-slate-900 bg-blue-50/30">
                                       {item.balance}
                                   </td>
                               </tr>
                           ))}
                           {inventoryData.length === 0 && (
                               <tr><td colSpan={4} className="p-8 text-center text-slate-500">No stock data available for selected project.</td></tr>
                           )}
                       </tbody>
                   </table>
               </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
               ...
               <div className="max-h-[500px] overflow-y-auto">
                     ...
                     .map(c => (
                       <div key={c.id} className="p-4 border-b border-slate-100 hover:bg-slate-50 group">
                           <div className="flex justify-between items-start mb-1">
                               ...
                               <button onClick={() => onDeleteConsumption(c.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                   <Trash2 size={14} />
                               </button>
                           </div>
                   ...
                   {consumption.length === 0 && (
                       <div className="p-8 text-center text-xs text-slate-400">No consumption records.</div>
                   )}
               </div>
           </div>
       </div>
